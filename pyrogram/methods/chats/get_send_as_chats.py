    ...
    async def get_send_as_chats(
        ...
        for p in r.peers:
            if isinstance(p.peer, raw.types.PeerUser):
                send_as_chats.append(await types.Chat._parse_chat(self, users[p.peer.user_id]))
            else:
                send_as_chats.append(await types.Chat._parse_chat(self, chats[p.peer.channel_id]))
